...
-- yeah, that
-- also patched to be fed clove pre made audio sources for convenience too
...
function TEsound.play( sound, sourceType, tags, volume, pitch, func )
  ...
  if not ( type(sound) == "string" or type(sound) == "userdata" or sound:type() == "SoundData" or type(sound) == 'Source' ) then
    error("You must specify a sound - a filepath as a string, a SoundData, a table of them or a source!")
  end
  
  if not type( sound ) == 'userdata' and sourceType ~= nil and not ( sourceType == 'stream' or sourceType == 'static' ) then
	error ("You must provide a sourceType (\"stream\" or \"static\") when not feeding an audio source directly.")
  end

  table.insert(TEsound.channels,
	{
		( type( sound ) == 'userdata' and sound or love.audio.newSource(sound, sourceType ) ),
		func,
