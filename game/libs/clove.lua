...
local otherExt = { 'xml', 'ini', 'json' }

local allExt={ "png","jpg","ttf","ogg","mp3","wav","otf","midi","jpeg","bmp","gif","vid", 'xml', 'ini', 'json' }
...
-- i'm stupid as hell, will return dashes in the names as well now
local function getValid(filename)
...
function clove.loadAsset(url,...)
	...
	if existsInList( otherExt, filetype ) then
		return love.filesystem.newFile( url )
	
	elseif existsInList(imgExt,filetype) then
	...
	for i=1,#dir do
		if isAFile(path..delim..dir[i]) then
			...
			if not except(dir[i]) and existsInList(assetType,getExtension(dir[i])) then
				...
				-- the problem is HEEEEEERRREEE, duplicate key names on this fcking table
				-- can't have file name in images and xmls/inis at the same time, THIS FUCKING CUNT
				if libName and not assetTbl[libName] then
					...
					elseif getExtension(dir[i]) == 'ini' then
						assetTbl[libName .. '.ini']=clove.loadAsset(path..delim..dir[i],param(dir[i]))
					else
...
function clove.loadOthers(...) return clove.load(otherExt, ...) end
